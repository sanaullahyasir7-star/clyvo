...
`npm test` includes regression checks for quota failures, stale-tab writes, backup recovery, unsupported imports, speech callback races, screen-capture cancellation and external end events. These browser-adapter tests use simulated APIs; they do not certify real microphone permissions or hardware support. Live now has optional on-device generative AI through WebLLM (Qwen2.5-1.5B-Instruct-q4f32_1-MLC), loaded only after a download click. The model runs in a Web Worker and streams answers using bounded CV and conversation context. It requires WebGPU and substantial memory; no cloud account or API key is used. Browser speech recognition may still use a vendor service. Live supports explicit meeting-tab audio capture in desktop Chrome 135+ via getDisplayMedia and SpeechRecognition.start(audioTrack). Users must select a browser tab and enable Share tab audio. Capture with no audio or a known non-tab surface is rejected and released. Older/unsupported browsers cannot silently fall back to microphone. Only one speech source is active; pause, end, source switching, unmount, errors, and browser stop release all tab tracks. Tab-capture video is not rendered or saved. Browser speech services may receive audio. Native desktop meeting applications remain unsupported. Template preparation and feedback remain deterministic. No semantic grading or screen interpretation is claimed. Model loading and generation remain experimental until validated on the user’s GPU.

Tab-audio regression tests cover source routing, startup status, stale callbacks, cancelled pickers, missing audio, wrong sources, and cleanup. They simulate media APIs and do not certify a real call or speech service.
